...
func TestAllCombosPrime(t *testing.T) {
	...
	for _, testCase := range testCases {
		for i := 0; i < len(testCase.primeIndexes); i++ {
			if testCase.primes[i] != primey.Nth(testCase.primeIndexes[i]) {
				t.Errorf("ERROR: For %v, %v expected %d, got %d", testCase.primeIndexes, testCase.primes, testCase.primes[i], primey.Nth(testCase.primeIndexes[i]))
			}
		}
	}
